...
- **Supabase keep-alive workflow.** `.github/workflows/keepalive.yml` pings the Supabase REST gateway twice a week (once the `KEEPALIVE_SUPABASE_URL` / `KEEPALIVE_SUPABASE_ANON_KEY` Actions secrets are set) so the free-tier project no longer auto-pauses after 7 idle days. A July 2026 outage traced to exactly this: the paused project's DNS record disappeared, so static pages kept returning 200 while sign-in and every protected route silently died. Diagnosis + restore steps documented in `docs/runbooks/supabase-setup.md` § Free-tier auto-pause and cross-linked from `deploy.md`.

- **Public landing page at `/`.** The root no longer auto-redirects into the app: it presents the VASEY/AI hero identity (eyebrow, IK monogram + gradient wordmark, version pill read from `package.json`, tagline, radial violet glow), a short value statement, three feature highlight cards (hybrid tracing engines, Claude-powered analysis, SVG export), and a single CTA — `Sign in to start` → `/login`, or `Enter Studio` → `/studio` when a valid session exists. Entering the studio is always an explicit action.
- **Working navigation drawer.** The mobile hamburger previously toggled orphaned store state that no component rendered (and the flag defaulted to *open*, inverting the first tap). New `NavDrawer` consumes that state properly: glass slide-in panel with focus trap, Esc/backdrop close, body scroll lock, and `role="dialog"` ARIA. Menu items are auth-aware — signed out: Home, Sign in; signed in: Home, Studio, Sign out with the account email in muted JetBrains Mono. Covered by a regression test (`tests/navDrawer.test.tsx`).
- **Visible Sign out** in both the drawer and the studio header (which now reflects the real Supabase session instead of never-populated store state). Logout clears the session and routes to `/`.
- **Suite hero + section treatment in the studio.** Compact hero above the mobile upload view, `UPLOAD`/`CONVERT` letterspaced section labels with thin rules, glass dropzone (glowing accent glyph, `DRAG & DROP IMAGE` display headline, `or tap to browse`, JetBrains Mono format chips), ≥44 px touch targets (52 px primary actions on mobile) with visible pressed states, and a global `prefers-reduced-motion` override plus `MotionConfig reducedMotion="user"` for the studio's framer-motion trees.

### Changed

- **Routing restructure.** Convert experience moved `/convert` → `/studio` (old URL redirects); sign-in moved `/auth` → `/login` (old URL redirects, `/auth/callback` unchanged); the OAuth callback now lands on `/studio` with the middleware still bouncing pending/suspended accounts to their status pages. Middleware redirects unauthenticated users to `/login` and leaves `/` fully public. PWA `start_url` was already `/` and now opens the landing page.
- **Sign-in is OAuth-only** (Google / GitHub / Microsoft via Supabase's `azure` provider); the email magic-link form was removed. The admin approval gate is unchanged.
- **Footer polish.** App version read from `package.json` instead of hard-coded; V/AI monogram box enlarged to 56 px to optically match the VM mark (its glyph only fills ~55 % of its viewBox).

- **VASEY/AI universal branded footer.** `src/components/layout/Footer.tsx` (shared by the `/convert`, `/design`, and `/admin` layouts) now renders the VASEY/AI App Suite footer template v1.1 — VM and V/AI SVG monograms linking to vaseymultimedia.com and vasey.ai, suite tag, app tag (`IkoniK v0.1.0 · Claude-Powered Vector Graphics Studio`), and copyright block. Template's required custom properties (`--accent-primary`, `--accent-dim`, `--accent-deep`, `--border-glow`, `--text-muted`) are aliased onto the existing IkoniK palette in `globals.css`; `'Space Mono'` swapped for the already-loaded JetBrains Mono.
- **iOS safe-area support.** `viewport-fit=cover` set via the root `viewport` export, with new `globals.css` utilities (`px-safe-4/6`, `pb-safe-6`, `h-safe-header`, `pt-safe-header[-loose]`) applied to the fixed Header, the Footer, and all three layout `<main>` offsets so content clears the notch, home indicator, and landscape sensor housing on iPhone/iPad PWA + Safari. Each utility declares a plain-value fallback first, so browsers without `env()`/`max()` keep the original spacing.
...
- **`CLAUDE.md` synced with the current model line-up** — the Thinking & Planning section referenced Opus 4.7 after the ADR 0002 bump; it now reads Opus 4.8, matching the Model Configuration section. The `svg-pipeline-reviewer` agent's `model` pin updated `claude-opus-4-7` → `claude-opus-4-8` for the same reason.
- **`next.config.mjs`** — `experimental.serverComponentsExternalPackages` migrated to the stable `serverExternalPackages` key (Next 15), clearing the "Invalid next.config.mjs options" build warning.
